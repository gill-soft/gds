...
public class OrderRequestValidator {
	...
	public void validateOrderRequest(OrderRequest request) {
		...
		for (int i = 0; i < request.getServices().size(); i++) {
			...
			// проверяем выбранные сервисы
			String servicesError = validateSelectedServices(item);
			if (!servicesError.isEmpty()) {
				errorMsg.append(servicesError).append("Service index = ").append(i).append("\r\n");
			}
			// проверяем кастомеров
			if (item.getCustomer() == null) {
				errorMsg.append("Empty customer\n");
			} else if (item.getCustomer().getId() == null) {
	...
	private String validateSelectedServices(ServiceItem item) {
		if (item.getSegment() == null
				&& item.getAdditionalService() == null) {
			return "Empty segment or additional service.";
		}
		StringBuilder errorMsg = new StringBuilder();
		if (item.getSegment() != null
				&& item.getSegment().getId() == null) {
			errorMsg.append("Segment part is present but empty id property.");
		}
		if (item.getAdditionalService() != null
				&& item.getAdditionalService().getId() == null) {
			errorMsg.append("Additional service part is present but empty id property.");
		}
