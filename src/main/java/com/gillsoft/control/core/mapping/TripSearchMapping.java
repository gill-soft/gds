package com.gillsoft.control.core.mapping;
...
public class TripSearchMapping {
	...
	@Autowired
	private List<SegmentAdditionalServiceMapper> additionalServiceMappers;
		...
		for (Entry<String, Segment> entry : searchResponse.getSegments().entrySet()) {
			try {
				...
				try {
					...
					DataConverter.applyLang(segment.getPrice().getTariff(), request.getLang());
				} catch (Exception e) {
					if (onlyCalculated) {
						continue;
					}
				}
				// добавление допуслуг к сегменту
				for (SegmentAdditionalServiceMapper additionalServiceMapper : additionalServiceMappers) {
					additionalServiceMapper.addServices(request, result, segment);
				}
	...
	public static interface SegmentAdditionalServiceMapper {
